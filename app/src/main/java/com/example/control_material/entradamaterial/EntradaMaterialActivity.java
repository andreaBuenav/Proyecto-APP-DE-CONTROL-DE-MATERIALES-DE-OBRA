...
public class EntradaMaterialActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvPrecioMaterial = findViewById(R.id.tvPrecioMaterial);
        ...
        btnHistorial.setOnClickListener(v -> {
            startActivity(
                    new android.content.Intent(
                            this, HistorialEntradaActivity.class
                    )
            );
        });
    ...
    private void cargarMateriales() {
        ...
        Cursor cursor = db.rawQuery(
                "SELECT material_id, nombre FROM material", null
        );
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<>(this,
                android.R.layout.simple_spinner_item, listaMateriales
        );
    ...
    private void mostrarStock(int position) {
        ...
        try {
            ...
            Cursor cursor = db.rawQuery(
                    "SELECT stock_actual FROM material WHERE material_id = ?", new String[]{String.valueOf(materialId)}
            );
    ...
    //obtener precio material
    private void obtenerPrecioMaterial() {

        String materialSeleccionado =
                spMaterial.getSelectedItem().toString();
        ...
        Cursor cursor = db.rawQuery(
                ...
                new String[]{materialSeleccionado}
        );

        if (cursor.moveToFirst()) {

            precioActual = cursor.getDouble(0);

            tvPrecioMaterial.setText(
                    "Precio: $" + precioActual
            );
        }
    ...
    private void guardarEntrada() {
        try {
            ...
            if (listaIds.isEmpty()) {
                Toast.makeText(
                        this, "No existen materiales registrados", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            if (cantidadTexto.isEmpty()) {
                Toast.makeText(
                        this, "Complete todos los campos", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            if (cantidad <= 0 ) {
                Toast.makeText(
                        this, "Cantidad y precio deben ser mayores a 0", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            long entradaId = db.insert(
                    "entrada_material", null, valuesEntrada);

            // VALIDAR INSERT

            if (entradaId == -1) {
                Toast.makeText(this, "Error al guardar entrada", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            valuesDetalle.put("precio_unitario", precioActual);
            long detalleId = db.insert(
                    "detalle_entrada", null, valuesDetalle
            );
            ...
            if (detalleId == -1) {
                ...
                Toast.makeText(
                        this, "Error al guardar detalle", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            db.execSQL(
                    "UPDATE material " + "SET stock_actual = stock_actual + ? " + "WHERE material_id = ?", new Object[]{cantidad, materialId}
            );

            // MENSAJE EXITO

            Toast.makeText(
                    this, "Entrada registrada correctamente", Toast.LENGTH_LONG).show();
            ...
            Toast.makeText(
                    this, "Ingrese valores numéricos válidos", Toast.LENGTH_SHORT).show();

        } catch (Exception e) {
            Toast.makeText(
                    this, "Error: " + e.getMessage(), Toast.LENGTH_LONG
            ).show();
